...
export class TodoListComponent implements OnInit {
  ...
  constructor(public fb: UntypedFormBuilder, private store: Store) {
  ...
  public orderBy2(condition: string) {
    ...
    if (condition === 'done') {
      ...
      this.items$ = this.store
        .select(ItemState.getAllItems)
        .pipe(map((value) => value.filter((task) => task.done === true)));
    }

    if (condition === 'active') {
      this.items$ = this.store
        .select(ItemState.getAllItems)
        .pipe(map((value) => value.filter((task) => task.done === false)));
    }
